---
description: Verifica el proyecto (build, tests y lógica repetida) y firma el resultado contra el diff staged.
allowed-tools: Bash
---
...
- Si algún paso sale en rojo, **no commitees**: arréglalo y vuelve a verificar. La firma
  solo vale para el diff exacto que había cuando se generó.
- Si sale el aviso de **lógica repetida**, míralo antes de seguir. No bloquea, pero si el
  ...
  después.
- Stagea y verifica en comandos SEPARADOS del commit. Encadenar `git add && git commit`
  cambia el diff entre la firma y el commit, y la puerta lo rechazará con razón.
